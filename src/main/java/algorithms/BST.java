package algorithms;
//Algorithm3.3 Binary Search tree symbol table
...
public class BST<Key extends Comparable<Key>, Value> {
	...
	// deleteMin(), deleteMax() and delete()
	...
	private Node delete(Node x, Key key) {
		...
		x.N = size(x.left) + size(x.right) +1;
		return x;// use the smallest Node in right sub tree to replace the delete node to maintain ordered
	}
	...
	private void keys(Node x, Queue<Key> queue, Key lo, Key hi) {
		...
		if (cmphi > 0) keys(x.right, queue, lo, hi);// three sub tree all goes down, pick all the keys that between lo and hi
	}
